...
public class OpenWnnJAJP extends OpenWnn {
    private final String TAG = OpenWnnJAJP.class.getSimpleName();
    /**
...
//                mConverterEN = new OpenWnnEngineEN("/data/data/jp.co.omronsoft.openwnn/writableEN.dic");
    ...
    public OpenWnnJAJP(Context context) {
        ...
        Log.d(TAG, "OpenWnnJAJP with Context");
    }
...
//            mConverterEN = new OpenWnnEngineEN("/data/data/jp.co.omronsoft.openwnn/writableEN.dic");
    ...
    public View onCreateInputView() {
        ...
        Log.d(TAG, "hiddenState: " + hiddenState);
...
//                boolean hidden = (hiddenState == Configuration.HARDKEYBOARDHIDDEN_YES);
    ...
    private boolean processKeyEvent(KeyEvent ev) {
        ...
        if (keyboardView != null) {
            if (keyboardView.isFocusFlag()) {
                Log.d(TAG, "SoftKeyboardView is focus");
                Keyboard.Key focusedKey = keyboardView.getFocusedKey();
                if ((KeyEvent.KEYCODE_DPAD_DOWN == key || KeyEvent.KEYCODE_DPAD_UP == key ||
                        KeyEvent.KEYCODE_DPAD_LEFT == key || KeyEvent.KEYCODE_DPAD_RIGHT == key)) {

                    Log.d(TAG, "getFocusedKey: " + focusedKey.codes[0]);
                    Keyboard.Key nextFocusedKey = keyboardView.getNearestKey(focusedKey, key);
                    if (nextFocusedKey != null) {
                        keyboardView.setFocusedKey(nextFocusedKey);
                        keyboardView.invalidate();
                    } else
                        Log.d(TAG, "nextFocusedKey is null");

                    return true;

                } else if (KeyEvent.KEYCODE_DPAD_CENTER == key) {
                    if (focusedKey != null)
                        ((DefaultSoftKeyboard) mInputViewManager).onKey(focusedKey.codes[0], null);

                    return true;
                }
            } else {
                if (mCandidatesViewManager.getCurrentView().isShown()) {
                    Log.d(TAG, "CandidatesView is isShown");
    ...
    private void processHardwareKeyboardSpaceKey(KeyEvent ev) {
        Log.d(TAG, "processHardwareKeyboardSpaceKey");
        /* H/W space key */
        if (ev.isShiftPressed()) {
    ...
    private void processHardwareKeyboardInputChar(StrSegment str) {
        Log.d(TAG, "processHardwareKeyboardInputChar");
        if (isEnableL2Converter()) {
...
//        }
    ...
    private void processRightKeyEvent() {
        Log.d(TAG, "processRightKeyEvent");
        if (mCandidatesViewManager.isFocusCandidate()) {
            mCandidatesViewManager.processMoveKeyEvent(KeyEvent.KEYCODE_DPAD_RIGHT);
            return;
        }
    ...
    boolean processKeyEventNoInputCandidateShown(KeyEvent ev) {
        Log.d(TAG, "processKeyEventNoInputCandidateShown");
        boolean ret = true;
        int key = ev.getKeyCode();
    ...
    private void updateViewStatusForPrediction(boolean updateCandidates, boolean updateEmptyText) {
        Log.d(TAG, "updateViewStatusForPrediction");
        EngineState state = new EngineState();
        state.convertType = EngineState.CONVERT_TYPE_NONE;
        updateEngineState(state);
    ...
    private void changeEngineMode(int mode) {
        ...
        switch (mode) {
            ...
            case OpenWnnEvent.Mode.NO_LV1_CONV:
                ...
                mEnableLearning = pref.getBoolean("opt_enable_learning_en", false);
                mEnablePrediction = pref.getBoolean("opt_prediction_en", false);
                if (OpenWnn.isXLarge()) {
                    mEnableSpellCorrection = pref.getBoolean("opt_spell_correction_en", false);
                } else {
            ...
            default:
                ...
                mEnableLearning = pref.getBoolean("opt_enable_learning_ja", false);
                mEnablePrediction = pref.getBoolean("opt_prediction_ja", false);
                break;
        }
    ...
    private void processSoftKeyboardToggleChar(String[] table) {
        Log.d(TAG, "processSoftKeyboardToggleChar");
        if (table == null) {
            return;
        }
    ...
    private void processSoftKeyboardCodeWithoutConversion(char[] chars) {
        Log.d(TAG, "processSoftKeyboardCodeWithoutConversion");
        if (chars == null) {
            return;
        }
    ...
    private void processSoftKeyboardCode(char[] chars) {
        Log.d(TAG, "processSoftKeyboardCode");
        if (chars == null) {
            return;
        }
    ...
    private void updateMetaKeyStateDisplay() {
        Log.d(TAG, "updateMetaKeyStateDisplay");
        int mode = 0;
        if (mHardShift == 0 && mHardAlt == 0) {
    ...
    private void commitConvertingText() {
        Log.d(TAG, "commitConvertingText");
        if (mEngineState.isConvertState()) {
    ...
    private void onKeyUpEvent(KeyEvent ev) {
        Log.d(TAG, "onKeyUpEvent: " + ev.toString());
        int key = ev.getKeyCode();
        if (!mShiftPressing) {
    ...
    private void initCommitInfoForWatchCursor() {
        Log.d(TAG, "initCommitInfoForWatchCursor");
        if (!isEnableL2Converter()) {
            return;
        }
    ...
    private void changeSymbolEngineState(EngineState state, int mode) {
        Log.d(TAG, "changeSymbolEngineState");
        state.temporaryMode = EngineState.TEMPORARY_DICTIONARY_MODE_SYMBOL;
        updateEngineState(state);
    }
...
//        mTextCandidatesViewManager.setEnableEmoticon(true);
    ...
    private boolean isHardKeyboard12KeyLongPress(int keyCode) {
        Log.d(TAG, "isHardKeyboard12KeyLongPress");
    ...
    private boolean processHardware12Keyboard(KeyEvent keyEvent) {
        Log.d(TAG, "processHardware12Keyboard");
        boolean ret = false;
